...
router.post('/', limiter, async function(req, res) {
  ...
  var courseList  = courses.map(function(c) { return '&bull; ' + c; }).join('<br>');
  ...
  var html = [
    ...
    '  </div>',
    (body.message ? [
      '  <div style="background:#fff;border-left:3px solid #d8d0c0;padding:1rem 1.4rem;margin-bottom:1rem;">',
      '    <p style="font-size:.75rem;font-family:monospace;letter-spacing:.1em;text-transform:uppercase;color:#7a7060;margin-bottom:.4rem;">Additional Notes</p>',
      '    <p style="margin:0;line-height:1.8;">' + body.message.replace(/\n/g, '<br>') + '</p>',
      '  </div>'
    ].join('') : ''),
